...
def finetune(model, train_data, dev_data, model_dir, ft_params):
    ...
    attention = SimpleAttention().to(encoder.device)  # <--- added attention
    ...
    for i in tqdm(range(ft_params.num_training_steps)):
        try:
            ...
            # Encode the sentence
            sent_encodings = encoder(**sents).last_hidden_state  # [B, T, H]

            # Apply attention to get reconstructed tokens
            attended_encodings, _ = attention(
                sent_encodings, goal_encodings
            )  # [B, T, H]

            # Compute token-level cosine loss
            token_losses = 1 - F.cosine_similarity(
                attended_encodings, goal_encodings, dim=-1
            )  # [B, T]

...
def finetune(model, train_data1, train_data2, dev_data, model_dir, ft_params):
    ...
    for i in tqdm(range(ft_params.num_training_steps)):
        try:
            encoder.eval()
            lang1_sents, lang2_sents, lang, goal_encodings = train_data1.next_batch()
            sent_mask = lang1_sents["attention_mask"].to(encoder.device)
            goal_mask = lang2_sents["attention_mask"].to(encoder.device)
            lang1_sents = lang1_sents.to(encoder.device)
            goal_encodings = goal_encodings.to(encoder.device)
            sent_encodings = encoder(**lang1_sents).last_hidden_state
            if lang != ("europarl", "es"):
                out1, _ = attn(sent_encodings, goal_encodings, sent_mask, goal_mask)
                token_scores1 = 1 - F.cosine_similarity(sent_encodings, out1, dim=-1)
                out2, _ = attn(goal_encodings, sent_encodings, goal_mask, sent_mask)
            ...
            def evaluate(dev_data):
                ...
                with torch.no_grad():
                    ...
                    while batch is not None:
                        sents, _, lang, goal_encodings = batch
